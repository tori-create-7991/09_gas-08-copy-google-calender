...
function formatDest(d) {
  ...
  if (d.inviteCopy != null) {
    lines.push(I3 + 'inviteCopy: {');
    var ic = d.inviteCopy;
    if (ic.enabled != null) lines.push(I3 + '  enabled: ' + ic.enabled + ',');
    if (ic.sourceCalendarId != null) lines.push(I3 + '  sourceCalendarId: ' + JSON.stringify(ic.sourceCalendarId) + ',');
    if (ic.daysBefore != null) lines.push(I3 + '  daysBefore: ' + ic.daysBefore + ',');
    if (ic.daysAfter != null) lines.push(I3 + '  daysAfter: ' + ic.daysAfter + ',');
    if (ic.eventTitle != null) lines.push(I3 + '  eventTitle: ' + JSON.stringify(ic.eventTitle) + ',');
    if (ic.eventColor != null) lines.push(I3 + '  eventColor: ' + ic.eventColor + ',');
    if (ic.showAsBusy != null) lines.push(I3 + '  showAsBusy: ' + ic.showAsBusy + ',');
    if (ic.includeOriginalLink != null) lines.push(I3 + '  includeOriginalLink: ' + ic.includeOriginalLink + ',');
    if (ic.maxCreatesPerRun != null) lines.push(I3 + '  maxCreatesPerRun: ' + ic.maxCreatesPerRun + ',');
    if (ic.sleepMsBetweenCreates != null) lines.push(I3 + '  sleepMsBetweenCreates: ' + ic.sleepMsBetweenCreates + ',');
    lines.push(I3 + '  rules: [');
    (Array.isArray(ic.rules) ? ic.rules : []).forEach(function(r, ri) {
      lines.push(I3 + '    {');
      if (r && r.name != null) lines.push(I3 + '      name: ' + JSON.stringify(r.name) + ',');
      ...
      lines.push(I3 + '    }' + (ri < ic.rules.length - 1 ? ',' : ''));
    });
    lines.push(I3 + '  ],');
    lines.push(I3 + '}');
  }
